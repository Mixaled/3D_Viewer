...
static void mouse_button_callback_in(App* app, GLFWwindow* window, int button,
                                     int action, int mods);
static void key_callback_in(App* app, GLFWwindow* window, int key, int scancode, int action, int mods);
...
void initialize_all(GLFWwindow** out_window, struct nk_context** nk_ctx,
  ...
  (*out_window) = glfwCreateWindow(START_WIN_WIDTH, START_WIN_HEIGHT,
                                   "3DViewer - Verdaqui, Sleepyka, Minellaa", NULL, NULL);
...
void key_callback(GLFWwindow* window, int key, int scancode, int action,
  ...
  key_callback_in(null, window, key, scancode, action, mods);
...
static void key_callback_in(App* app, GLFWwindow* window, int key, int scancode, int action, int mods) {
  ...
  if (current_app is_not null and window is_not null) {
    app_on_key_press(current_app, key, scancode, action, mods);
  }
}
