...
static void printer_printable(OutStream stream, VaListWrap* list,
                              int* total_written);
static void printer_matrix_t(OutStream stream, VaListWrap* list,
                              int* total_written);
static void printer_face(OutStream stream, VaListWrap* list,
                              int* total_written);

#define FORMATS \
  { "$printable", "$matrix_t", "$slice", "$face" }
#define PRINTERS \
  { printer_printable, printer_matrix_t, printer_slice, printer_face}
...
static void printer_matrix_t(OutStream stream, VaListWrap* list,
                              int* total_written) {
  matrix_t val = va_arg(list->list, matrix_t);
  s21_matrix_print(&val, stream);
  (*total_written) += 5;  // TODO
}

static void printer_face(OutStream stream, VaListWrap* list,
                              int* total_written) {
  Face val = va_arg(list->list, Face);
  face_print(&val, stream);
  (*total_written) += 5;  // TODO
}
