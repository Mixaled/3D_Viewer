...
GlProgram gl_program_from_2_paths(const char* path_vert, const char* path_frag) {
  Shader vert = shader_from_file(GL_VERTEX_SHADER, path_vert);
  Shader frag = shader_from_file(GL_FRAGMENT_SHADER, path_frag);

  GlProgram result = gl_program_from_2_shaders(&vert, &frag);
